...
+(UIImage *)generateQRCodeWithData:(NSString *)data {
    return [self getColorQRCode:data];
}

+(UIImage *)getColorQRCode:(NSString *)code{
    NSData *string_data = [code dataUsingEncoding:NSUTF8StringEncoding];
    // 1、二维码滤镜
    CIFilter *fileter = [CIFilter filterWithName:@"CIQRCodeGenerator"];
    [fileter setDefaults];
    [fileter setValue:string_data forKey:@"inputMessage"];
    CIImage *ciImage = [fileter outputImage];
    return [self createNonInterpolatedUIImageFormCIImage:ciImage size:440];
}
...
+ (UIImage*)imageBlackToTransparent:(UIImage*)image {
    ...
    for (int i = 0; i < pixelNum; i++, pCurPtr++){
        ...
        if ((*pCurPtr & 0xFFFFFF00) < 0x99999900)
        {
            ...
            if (row <= 52 && row >=24 && col <= 52 && col >= 24) {
                uint8_t* ptr = (uint8_t*)pCurPtr;
                ptr[3] = 250; //0~255
                ptr[2] = 0;
                ptr[1] = 0;
            }
            if (row <= 52 && row >=24 && col <= 416 && col >= 388) {
                uint8_t* ptr = (uint8_t*)pCurPtr;
                ptr[3] = 250; //0~255
                ptr[2] = 0;
                ptr[1] = 0;
            }
            if (col <= 52 && col >=24 && row <= 416 && row >= 388) {
                uint8_t* ptr = (uint8_t*)pCurPtr;
                ptr[3] = 0; //0~255
                ptr[2] = 121;
                ptr[1] = 229;
...
+ (UIImage *)generateQRCodeWithData:(NSString *)data size:(CGFloat)size color:(UIColor *)color backgroundColor:(UIColor *)backgroundColor {
    ...
    [color_filter setValue:ciImage forKey:kCIInputImageKey];
