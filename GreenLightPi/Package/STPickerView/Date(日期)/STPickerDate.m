...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    if (component == 0) {
        ...
        if (self.year == [NSCalendar currentYear] || self.year == _yearLeast) {
            return [NSCalendar currentMonth];
        }
        return 12;
    } else {
        if ((self.year == [NSCalendar currentYear] || self.year == _yearLeast) && self.month == [NSCalendar currentMonth]) {
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view
{
    ...
    NSString *text;
    if (component == 0) {
        text =  [NSString stringWithFormat:@"%ld", row + _yearLeast];
    }else if (component == 1){
