...
- (NSMutableArray *)reloadCalendarView:(NSDate *)date  selectDate:(NSDate *)selectdate selectLastDate:(NSDate *)selectLastdate needDays:(int)days_number
{
    //如果为空就从当天的日期开始
    if(date == nil){
        date = [NSDate date];
    ...
    if (selectLastdate == nil) {
        selectLaet = before;
    } else {
        selectLaet = selectLastdate;
    }
...
- (void)changStyle:(CalendarDayModel *)calendarDay
{
    ...
    NSDateComponents *calendarSelectLast = [selectLaet YMDComponents];//可选择的最后一天
    ...
    //没被点击选中
        ...
        //默认选择的第一天之前的时间设置一个灰度
        if ((calendarSelect.year > calendarDay.year) || (calendarSelect.year == calendarDay.year &
             calendarSelect.month > calendarDay.month) || (calendarSelect.year == calendarDay.year &
                                                           calendarSelect.month == calendarDay.month & calendarSelect.day > calendarDay.day)) {
...
//        //最后一天可选时间之后的时间段
//        }
            else if ((calendarSelectLast.year < calendarDay.year) || (calendarSelectLast.year == calendarDay.year &
                      calendarSelectLast.month < calendarDay.month) || (calendarSelectLast.year == calendarDay.year &
                                                                        calendarSelectLast.month == calendarDay.month & calendarSelectLast.day < calendarDay.day)
                  ) {
