...
- (void)initView{
    //选中时显示的图片
    imgview = [[UIImageView alloc]initWithFrame:CGRectMake(13, 23, self.bounds.size.width-26, self.bounds.size.width-26)];
//    imgview.image = [UIImage imageNamed:@"chack.png"];
    imgview.backgroundColor = KHEXRGB(0x44C08C);
    XSViewBorderRadius(imgview, (self.bounds.size.width-26)/2, 0, KHEXRGB(0x44C08C));
    ...
    switch (model.style) {
        ...
        case CellDayTypeFutur://将来的日期
            ...
            if (model.holiday) {
                ...
                day_lab.text = [NSString stringWithFormat:@"%lu",(unsigned long)model.day];
                day_lab.textColor = KHEXRGB(0x44C08C);
            }
