...
//火车初始化方法
- (void)setTrainToDay:(int)day ToDateforString:(NSString *)todate LastDateforString:(NSString *)lastdate
{
    daynumber = day;
    optiondaynumber = 1;//选择一个后返回数据对象
    super.calendarMonth = [self getMonthArrayOfDayNumber:daynumber ToDateforString:todate LastDateforString:lastdate];
    [super.collectionView reloadData];//刷新
}
...
//获取时间段内的天数数组
- (NSMutableArray *)getMonthArrayOfDayNumber:(int)day ToDateforString:(NSString *)todate LastDateforString:(NSString *)lastdate
{
    ...
    if (lastdate) {
        lastDate = [lastDate dateFromString:lastdate];
    } else {
        lastDate = nil;
    }
    ...
    return [super.Logic reloadCalendarView:date selectDate:selectdate selectLastDate:lastDate needDays:day];
