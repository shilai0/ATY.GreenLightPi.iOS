...
@interface FCStartView ()
{
    UILabel *animateLabel;//评分时动画显示分数
}
@end

@implementation FCStartView
...
- (void)setLevel:(float)level
{
    if (_level != level || level == 0) {
        ...
        //默认图标
        if (!_iconFull) {
            _iconFull = [UIImage imageNamed:@"comment_star_highlighted.png"];
            _iconHalf = [UIImage imageNamed:@"comment_star_half.png"];
            _iconEmpty = [UIImage imageNamed:@"comment_star.png"];
            ATYLog(@"GradeStartView：找不到属性iconFull对象，将使用默认风格图标！");
        }
        [self setNeedsDisplay];
    }
}
