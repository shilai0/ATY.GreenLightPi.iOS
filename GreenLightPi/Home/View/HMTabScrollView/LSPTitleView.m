...
- (UIView *)bottomLine{
    if (_bottomLine == nil) {
        ...
        XSViewBorderRadius(_bottomLine, 1, 0, KHEXRGB(0x44C08C));
    }
    return _bottomLine;
}
...
- (LSPTitleView *)initWithFrame:(CGRect)frame titles:(NSArray<NSString *> *)titles style:(LSPTitleStyle *)style{
    ...
    if (self = [super initWithFrame:frame]) {
        self.titles = titles;
        self.style  = style;
        self.currentIndex = self.style.index;
        [self setupUI];
    }
    return self;
}
...
- (void)setupTitleLabels{
    ...
    for (int i = 0; i < self.titles.count; i++) {
        ...
        label.textColor = i == self.style.index ? self.style.selectedColor : self.style.normalColor;
...
- (void)setupTitleLabelFrame{
    CGFloat titleX = 17.0;
    CGFloat titleY = 0.0;
    CGFloat titleW = self.frame.size.width/self.titleLabels.count;
    CGFloat titleH = self.frame.size.height;
    
    for (int i = 0; i < self.titleLabels.count; i++) {
        UILabel *label = self.titleLabels[i];
        if (!self.style.isAverage) {
            CGSize strSize = [self sizeWithFontNumber:self.style.font limitedInHeight:44 withString:label.text];
            titleW = strSize.width;
            
            if (i == 0) {
                titleX = 17;
            }else{
                UILabel *preLabel = self.titleLabels[i - 1];
                titleX = CGRectGetMaxX(preLabel.frame) + self.style.adjacentMargin;
            }
            label.frame = CGRectMake(titleX, titleY, titleW, titleH);
        } else {
            label.frame = CGRectMake(titleW * i, titleY, titleW, titleH);
        }
        
        //放大的代码
        if (i == self.style.index) {
...
- (void)setupBottomLine{
    ...
    UILabel *defaultLabel = [self.titleLabels objectAtIndex:self.style.index];
    self.bottomLine.frame = CGRectMake(defaultLabel.frame.origin.x + ((defaultLabel.frame.size.width - self.style.bottomLineW) / 2.0), self.bounds.size.height - self.style.bottomLineH, self.style.bottomLineW, self.style.bottomLineH);
}

- (void)setupCoverView{
    [self.scrollView addSubview:self.coverView];
    
    [self.scrollView insertSubview:self.coverView atIndex:self.style.index];
...
- (void)titleLabelClick:(UITapGestureRecognizer *)tap{
    ...
    [self updateStyle:currentLabel];
}

- (void)switchTitleWithIndex:(NSInteger)index {
    UILabel *currentLabel = (UILabel *)[self viewWithTag:10 + index];
    //如果重复点击同一个title，那么直接返回
...
- (void)updateStyle:(UILabel *)currentLabel {
    //获取之前的Label
...
- (void)setTitleWithProgress:(CGFloat)progress sourceIndex:(NSInteger)sourceIndex targetIndex:(NSInteger)targetIndex{
    ...
    UILabel *endLabel = [self.titleLabels lastObject];
    if ((endLabel.frame.origin.x + endLabel.frame.size.width) <= self.frame.size.width) {
...
- (CGSize)sizeWithFontNumber:(UIFont *)font limitedInHeight:(CGFloat)height withString:(NSString *)string {
    CGSize strSize = [string boundingRectWithSize:CGSizeMake(1024, height) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:font} context:nil].size;
